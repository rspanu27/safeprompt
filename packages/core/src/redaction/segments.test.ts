...
import { splitRedacted } from './segments';

describe('splitRedacted', () => {
  it('marks the placeholders the scan inserted', () => {
    expect(splitRedacted('key=[SECRET_1] host=[HOST_1]', ['[SECRET_1]', '[HOST_1]'])).toEqual([
  ...
  it('leaves bracketed text alone when the scan did not insert it', () => {
    expect(splitRedacted('see [NOTE_1] above', [])).toEqual([
      { text: 'see [NOTE_1] above', token: false },
    ]);
  });

  it('does not match a shorter token inside a longer one', () => {
    const parts = splitRedacted('[EMAIL_10] and [EMAIL_1]', ['[EMAIL_1]', '[EMAIL_10]']);
